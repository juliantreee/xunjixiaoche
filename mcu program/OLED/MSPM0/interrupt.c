...
{
    // 1. 处理GPIO多源中断（传感器数据就绪）
    switch (DL_Interrupt_getPendingGroup(DL_INTERRUPT_GROUP_1)) {
        #if defined GPIO_MULTIPLE_GPIOA_INT_IIDX  // 若GPIOA存在多源中断
        case GPIO_MULTIPLE_GPIOA_INT_IIDX:
            ...
            switch (DL_GPIO_getPendingInterrupt(GPIOA)) {
                ...
                case GPIO_VL53L0X_PIN_VL53L0X_GPIO1_IIDX:
                    Read_VL53L0X();  // 读取VL53L0X数据
                    break;
                #endif
                default:
                    break;
            }
            DL_GPIO_clearInterruptFlag(GPIOA, DL_GPIO_ALL_PINS);  // 清除GPIOA所有中断标志
            break;
        #endif

        #if defined GPIO_MULTIPLE_GPIOB_INT_IIDX  // 若GPIOB存在多源中断
        case GPIO_MULTIPLE_GPIOB_INT_IIDX:
            ...
            DL_GPIO_clearInterruptFlag(GPIOB, DL_GPIO_ALL_PINS);  // 清除GPIOB所有中断标志
            break;
        #endif
